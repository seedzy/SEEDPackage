...
namespace CSV2Mesh
{
    public class CSV2Mesh : EditorWindow
    {
        ...
        private static string MeshName = "New Mesh";
        ...
        //static int data_uv2_id = 18;

        [MenuItem("SEEDTools/CSV2Mesh")]
        static void Open()
        {
        ...
        private void OnGUI()
        {
            ...
            if (dataTypes)
            {
                ...
                //data_uv2_id = EditorGUILayout.Popup("UV2: ", data_uv2_id, Head.ToArray(), EditorStyles.popup);

                MeshName = dataTypes.name;
                ...
                {
                    //Debug.LogWarning(textAssets[0].name);
                }
        ...
        public static int ImportMeshFromRenderDoc(string assetPath, bool needRotationN90, bool reverseUvY, bool reverseTris, List<string> head)
        {
            ...
            var rotationN90 = needRotationN90 ? Quaternion.Euler(0, -90, -90) : Quaternion.identity;
            for (int i = 0; i < allRows.Count; ++i)
            {
                ...
                if (realIndex < outputVertexs.Length && realIndex >= 0)
                {
                    ...
                    if (hasTangentProp)
                    {
                         var tan = new Vector4(currLine[tangent_x], currLine[tangent_y], currLine[tangent_z], currLine[tangent_w]);
                         outputTangents[realIndex] = rotationN90 * tan;
                    }
