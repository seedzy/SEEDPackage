...
half3 DirectDiffuseWithoutAlbedo(ToonSurfaceData surfaceData, InputData inputData, Light light, half NdotL, half2 rampV)
{
    ...
    if(shadowAttenuation < _LightArea && shadowWeight < 0.95)
    {
        ...
        if(time)
        {
            ...
            rampColor = _StockingRamp.Sample(sampler_StockingRamp, float2(shadowAttenuation, 0.5));
        #else
...
half3 SetStockingColor(half stockingMask, half3 albedo, InputData inputData)
{
    #ifdef _USE_SILKSTOCKING
    half NdotV = saturate(dot(inputData.normalWS, inputData.viewDirectionWS));
    //half fresnelTerm =  Fresnel(NdotV, _StockingPow) * stockingMask;
    //暂时用b通道来表示denier值
    half fresnelTerm =  pow(NdotV, _StockingPow) * saturate(albedo.b * _StockingDenier);;
    half3 stockingColor = lerp(_StockingColor , albedo * _StockingSubColor, fresnelTerm);
    albedo = lerp(albedo, stockingColor, stockingMask);
    //albedo = lerp(albedo, albedo.g, stockingMask);
    #endif
    return albedo;
}
...
// #endif
    ...
    else
    {
        ...
        half3 diffuse = (IndirectDiffuse + directDiffuse) * SetStockingColor(surfaceData.maskMap.r, surfaceData.albedo, inputData);
...
float RimLight(float2 posNDC_zw, float4 positionSSWithOutDivideW, half threshold, half NdotL)
{
    //unity的深度图不是线性的，计算公式如下
    //_CameraDepthTexture.r = input.positionNDC.z / input.positionNDC.w
    float depth = posNDC_zw.x / posNDC_zw.y;
    float linearEyeDepth = LinearEyeDepth(depth, _ZBufferParams);
    float depth_Tex = SampleSceneDepth(positionSSWithOutDivideW.xy / positionSSWithOutDivideW.w);
    float linearEyeDepth_Tex = LinearEyeDepth(depth_Tex, _ZBufferParams);
    //迎光面边缘光
    float depthDiff = (linearEyeDepth_Tex - linearEyeDepth);
    return step(threshold, depthDiff) * NdotL;
    //float linear
}
