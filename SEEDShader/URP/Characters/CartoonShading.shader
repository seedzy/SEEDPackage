...
//        _OcclusionRemapEnd("_OcclusionRemapEnd", Range(0,1)) = 1
        ...
        _Metal_SpecPower("MetalSpecularShininess", range(0.01, 100)) = 90
        ...
        [Header(Outline)]
        [Toggle(_SMOOTHFROMTANGENT)]_SmoothFromTangent("_SmoothFromTangent", float) = 0
        ...
        _OutlineColor1("_OutlineColor1", Color) = (1,0,0,1)
        _OutlineColor2("_OutlineColor2", Color) = (0,1,0,1)
        _OutlineColor3("_OutlineColor3", Color) = (0,0,1,1)
        _OutlineColor4("_OutlineColor4", Color) = (1,1,0,1)
        _OutlineColor5("_OutlineColor5", Color) = (0,1,1,1)
        ...
        [Header(RimLighting)]
        _RimWidth("RimWidth", Range(0, 10)) = 0.012
        _Threshold("Threshold", Range(0, 1)) = 0.09
        
        [Header(Advanced Option)]
        _MaskMapAtlas("MaskMapAtlas", 2D) = "Black" {}
        [Toggle(_USE_SILKSTOCKING)]_UseStocking("UseStocking", float) = 0
        _StockingRamp("StockingRamp", 2D) = "White" {}
        _StockingColor("StockingColor", Color) = (0,0,0,0)
        _StockingPow("StockingPow", Range(0,100)) = 0.5
        _StockingDenier("StockingDenier", Range(0,5)) = 1
        _StockingSubColor("StockingSubColor(增强肉色)", color) = (1,1,1,1)
    ...
    {       
        ...
        {               
            ...
            #pragma shader_feature_local_fragment _USE_SILKSTOCKING
            #pragma shader_feature_local _SMOOTHFROMTANGENT
...
//        // For example, when depth texture is on, we need to perform this offscreen depth prepass for this toon shader. 
        Pass
        {
            ...
            ZTest LEqual // early exit at Early-Z stage if possible            
            ColorMask 0 // we don't care about color, we just want to write depth, ColorMask 0 will save some write bandwidth
            Cull [_Culling] // support Cull[_Cull] requires "flip vertex normal" using VFACE in fragment shader, which is maybe beyond the scope of a simple tutorial shader
